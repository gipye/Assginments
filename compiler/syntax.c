...
//check function delcarator and return type
A_ID *setFunctionDeclaratorSpecifier(A_ID *id, A_SPECIFIER *p) {
	...
	if(id->type == NULL || id->type->kind != T_FUNC) {
		syntax_error(21);
		return(id);
	}
	else {
	...
	a = id->type->field;
	if(a != NIL && a->type == void_type) {
		if(a->link != NIL || strlen(a->name))
			syntax_error(14);
		id->type->field = NIL;
		return id;
	}
...
void syntax_error(int i, char *s) {
	...
	switch(i) {
		case 11:
			printf("illegal referencing struct or union identifier %s", s); break;
		case 12:
		...
		case 21:
			printf("illegal function delcarator"); break;
		case 22:
			printf("conflicting parm type in prototype function %s", s); break;
		case 23:
			printf("empty parameter name"); break;
		case 24:
			printf("illegal decalration specifiers"); break;
		case 25:
